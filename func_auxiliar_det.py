...
def simula_detalhada_base(simCase, config, inputs):
    ...
    carga_GASDUC = MaterialStreams["GASDUC-II"].MolarFlow.GetValue("m3/d_(gas)")
    carga_MIX_UTGCAB = MaterialStreams["MIX UTGCAB"].MolarFlow.GetValue("m3/d_(gas)")
    carga_UPGN_II = (
        MaterialStreams["BypassU211"].MolarFlow.GetValue("m3/d_(gas)")
        + MaterialStreams["URCO2toU211"].MolarFlow.GetValue("m3/d_(gas)")
        + MaterialStreams["299toUPGN"].MolarFlow.GetValue("m3/d_(gas)")
        + MaterialStreams["299toUPGN"].MolarFlow.GetValue("m3/d_(gas)")
    )
    carga_URGN = MaterialStreams["Carga URGN"].MolarFlow.GetValue("m3/d_(gas)")
    ...
    # Temp_V03 = MaterialStreams_UPGN['Fundo do V-03'].Temperature.Getvalue("C")
...
def simula_detalhada(simCase, config, inputs, cargas):
    ...
    GV_energia = GV_Fv_net * GV_PCS2  # MMbtu/d
    GV_preco = inputs["otimizacao"]["preco"]["custoOportunidade"]["GV"][
        "valor"
    ]  # [R$/MMbtu]
    GV_receita = GV_preco * GV_energia  # [R$/d]
    # Receita LGN
    ...
    LGN_PCS2 = LGN_PCS / LGN_Vm  # MMbtu/m3
    LGN_energia = LGN_Fv * LGN_PCS2  # MMbtu/d
    LGN_preco = inputs["otimizacao"]["preco"]["custoOportunidade"]["LGN"][
        "valor"
    ]  # [R$/MMbtu]
    LGN_receita = LGN_preco * LGN_energia  # [R$/d]
    # Receita GLP
    ...
    GLP_PCS2 = GLP_PCS / GLP_Vm  # MMbtu/m3
    GLP_energia = GLP_Fv * GLP_PCS2  # MMbtu/d
    GLP_preco = inputs["otimizacao"]["preco"]["custoOportunidade"]["GLP"][
        "valor"
    ]  # [R$/MMbtu]
    GLP_receita = GLP_preco * GLP_energia  # [R$/d]
    # Receita C5+
    ...
    C5p_PCS2 = C5p_PCS / C5p_Vm  # MMbtu/m3
    C5p_energia = C5p_Fv * C5p_PCS2  # MMbtu/d
    C5p_preco = inputs["otimizacao"]["preco"]["custoOportunidade"]["C5+"][
        "valor"
    ]  # [R$/MMbtu]
    C5p_receita = C5p_preco * C5p_energia  # [R$/d]
    # Receita_total
